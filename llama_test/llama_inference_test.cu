...
public:
    ...
    bool testBigGemm_v2() {
        printf("\n=== Testing Big GEMM (%dx%d mul %dx%d) ===\n", N, N * k, N*k, N);
    ...
    bool testNaiveGemm_v2() {        
        int batch_size = k;
        int m = 1;          // Height of A and C
        int n = N;          // Width of B and C  
        int k_dim = 128;    // Width of A and height of B
        
        printf("\n=== Testing Naive GEMM (%d individual %dx%d mul %dx%d) ===\n", k, m, k_dim, k_dim, n);
    ...
    bool testBatchedGemm_v2() {        
        ...
        printf("\n=== Testing Batched GEMM (%d batches of %dx%d mul %dx%d) ===\n", k, 1, k_dim, k_dim, n);
    ...
    bool testStridedBatchedGemm_v2() {        
        ...
        printf("\n=== Testing Strided Batched GEMM (%d batches of %dx%d mul %dx%d) ===\n", k, 1, k_dim, k_dim, n);
