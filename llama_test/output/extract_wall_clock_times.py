...
def extract_timing_data(txt_file):
    ...
    for test_run in test_runs[1:]:  # Skip first empty split
        ...
        cmd_match = re.search(r'./llama_test\s+(\d+)\s+(\d+)', test_run)
        if not cmd_match:
            continue
...
def main():
    ...
    txt_file = '/home/ubuntu/dbs/llm/llm.c/llama_test/comparison.txt'
    timing_data = extract_timing_data(txt_file)
    
    # Load existing CSV
    csv_file = '/home/ubuntu/dbs/llm/llm.c/llama_test/output/result_wide.csv'
    ...
    output_file = '/home/ubuntu/dbs/llm/llm.c/llama_test/output/result_wide_with_timing.csv'
