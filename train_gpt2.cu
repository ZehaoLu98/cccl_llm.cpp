...
void gpt2_forward(GPT2 &model, int* inputs, int* targets, int B, int T) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        #if !defined(PROFILE_SEPERATE_ATTENTION) && defined(PROFILE_FORWARD)
        ...
        #if !defined(PROFILE_SEPARATE_MLP) && defined(PROFILE_FORWARD)
        ...
        // #endif
        // vecAdd<<<6144*2, 256>>>(l_residual3, l_residual2, l_fcproj, B*T*C);
        // #ifdef PROFILE_FORWARD
        // if(l==1 && counter == 1)GmpProfiler::getInstance()->popRange("vecadd", GmpProfileType::CONCURRENT_KERNEL);
        // #endif

        // #ifdef PROFILE_FORWARD
        // if(l==1 && counter == 1)GmpProfiler::getInstance()->pushRange("vecadd_thrust", GmpProfileType::CONCURRENT_KERNEL);
        // #endif
        // vecAdd_thrust(l_residual3, l_residual2, l_fcproj, B*T*C);
        // #ifdef PROFILE_FORWARD
        // if(l==1 && counter == 1)GmpProfiler::getInstance()->popRange("vecadd_thrust", GmpProfileType::CONCURRENT_KERNEL);
        // #endif
    }
...
int main(int argc, char *argv[]) {
    ...
    int train_num_batches = num_steps < train_loader.num_batches?num_steps:train_loader.num_batches;
